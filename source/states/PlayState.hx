...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var songBy:FlxSprite;
		songBy = new FlxSprite().loadGraphic(Paths.image('bars/barra')); //Image / Imágen
		var songByTimer:FlxTimer;
		//songBy.screenCenter(X);
		songBy.x = 3450;
		songBy.y = 200;
		FlxTween.tween(songBy, {x: -10}, 0.7, {ease: FlxEase.backInOut}); //Animation / Animation
		songByTimer = new FlxTimer().start(2, function(tmr:FlxTimer) { //Animation / Animation
			FlxTween.tween(songBy, {x: 1600}, 0.7, {ease: FlxEase.backInOut}); //Animation / Animation
		});
		add(songBy);

		var songByTxt:FlxText;
		songByTxt = new FlxText(200, 250, FlxG.width, "", 56);
		songByTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songByTxt.scrollFactor.set();
		songByTxt.borderSize = 1.25;
		songByTxt.size = 56;
		songByTxt.y = 250;
		songByTxt.x = 200;
		songByTxt.text = SONG.song;
		FlxTween.tween(songByTxt, {x: -500}, 0.7, {ease: FlxEase.backInOut}); //Animation / Animation
		songByTimer = new FlxTimer().start(2, function(tmr:FlxTimer) { //Animation / Animation
			FlxTween.tween(songByTxt, {x: 1500}, 0.7, {ease: FlxEase.backInOut}); //Animation / Animation
		});
		add(songByTxt);
